...
**Monitor loop** (`MonitorService`) runs on a background `Task`, polling every 5 seconds. It accumulates session time into today's `DailyLog` and fires events (`BlockRequested`, `WarnRequested`) back to the UI thread. `App.xaml.cs` handles those events — it kills the process and shows the appropriate popup.
...
**Storage:** All data lives under `%AppData%\TimeGuard\`. `config.json` holds rules and the password hash. A new `logs/YYYY-MM-DD.json` is created each day and read at startup. Midnight rollover is handled in the monitor loop.
...
- `RulesEngine` never reads from disk or kills processes. All side effects live in `MonitorService` (disk) and `App.xaml.cs` (process kill + UI).
